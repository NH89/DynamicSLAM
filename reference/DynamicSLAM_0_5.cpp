...
protected:
    int                             appFrameNum;
    bool nextFrame(cv::UMat& frame) { 
        appFrameNum++;
        cout<<"appFrameNum="<<appFrameNum;  
...
private:  
    ...
    vector<PredictionImageColumn> prediction;                               //numPredictions i.e. on-target and cluster around it.
    #define    NUM_IMCOLS 6 // 
    // windows
...
cout<<"App::App constructor  m_file_name=" << m_file_name << ".\n";
    ...
    prediction.reserve(NUM_IMCOLS );                                                   // numPredictions i.e. on-target and cluster around it.
...
{
    // whole image first
    //start from predicted images - expected 6D transform & one step in each rotation (& translation)
    // find current photometric cost & gradient of cost
    // calc next sample transform by Levenberg-Marquard (or chk methods in DTAM & DynamicSLAM)
    // nb layers & channels of collumn, ? which 
    
    // Then find pixelwise relative velocity map 
    // TV_L1 smoothing of depth map
    
    
    // Mapping
    // How far does relative depth parrallax motion explain velocity map ? 
    // TV_L1 smoothing of depth map
    // Relative depth has lower cost than relative motion (motion = difference in both position and velocity)
    
    // Relative scale estimation - for independently moving objects 
    // Detect camera acceleration signal present in all regions => common basis for depth
...
cout<<"Showing compositeImg, "<<endl<<flush; // nb need to make frame num an app variable (coppied to imcol variable if needed)
